...
def delete_old_messages(
    ...
    """Delete messages older than N days from a folder."""
...
def archive_by_sender(
    ...
    _batch_label(client, ARCHIVE, messages, action="archive")
...
def handle_newsletters(
    ...
    """Detect newsletters and optionally delete them."""
    ...
    if not confirm_action("delete detected newsletters", len(newsletters), dry_run):
        if not dry_run:
            print_warning("Cancelled.")
        return

    _batch_delete(client, newsletters)
...
def _batch_label(
    ...
    """Apply a label to messages in batches."""
    ids = [m.get("ID", m.id if hasattr(m, "id") else m) for m in messages]
    total = len(ids)
